import { Wallet, LineChart, Fuel, Car } from "lucide-react"
import EmptyCar from "../assets/emptyCar.png"

const ComparisonCard = ({ car, placeholder, onAdd }) => {
  const isEmpty = !car

  return (
    <div
      className={`
        rounded-3xl border-2 p-6 sm:p-8 min-h-[420px]
        flex flex-col lg:flex-row gap-6 lg:gap-8
        transition relative
        ${isEmpty
          ? "border-gray-200 bg-white hover:border-blue-300 cursor-pointer"
          : "border-blue-400 bg-white"
        }
      `}
      onClick={isEmpty ? onAdd : undefined}
    >
      {/* LEFT */}
      <div className="flex-1 flex flex-col justify-between">
        {/* Image */}
        <div className="flex justify-center">
          {isEmpty ? (
            <img
              src={EmptyCar}
              alt="Empty Car Slot"
              className="h-32 sm:h-40 lg:h-44 object-contain opacity-50"
            />
          ) : (
            <img
              src={car.image}
              alt={car.name}
              className="h-32 sm:h-40 lg:h-44 object-contain"
            />
          )}
        </div>

        {/* Text */}
        <div className="mt-6">
          {isEmpty && (
            <button className="text-blue-600 text-sm font-semibold mb-3">
              Add Car
            </button>
          )}

          <h2 className="text-2xl sm:text-3xl font-bold text-gray-900">
            {isEmpty ? placeholder : car.name}
          </h2>

          <p className="text-gray-500 mt-1 text-sm sm:text-base">
            {isEmpty
              ? "Internal Combustion Engine (ICE)"
              : car.type === "EV"
              ? "Electric Vehicle (EV)"
              : "Internal Combustion Engine (ICE)"}
          </p>

          {!isEmpty && (
            <button
              onClick={(e) => {
                e.stopPropagation()
                onAdd()
              }}
              className="text-blue-600 text-sm font-semibold mt-3"
            >
              Change car
            </button>
          )}
        </div>
      </div>

      {/* RIGHT METRICS */}
      <div className="flex flex-col gap-4 w-full lg:w-56">
        <Metric
          title="Total Cost Ownership"
          value={isEmpty ? "-" : "GHS 241,120"}
          icon={Wallet}
          active={!isEmpty}
        />
        <Metric
          title="Tailpipe Emissions"
          value={isEmpty ? "-" : "0.00 kgCO₂e"}
          icon={LineChart}
          active={!isEmpty}
        />
        <Metric
          title="Fuel Economy"
          value={isEmpty ? "-" : "GHS 241"}
          icon={Fuel}
          active={!isEmpty}
        />

        {/* Floating icon */}
        <div className="flex justify-end pt-2">
          <div
            className={`
              w-10 h-10 rounded-full flex items-center justify-center
              ${isEmpty ? "bg-gray-100" : "bg-blue-100 text-blue-600"}
            `}
          >
            <Car size={18} />
          </div>
        </div>
      </div>
    </div>
  )
}

const Metric = ({ title, value, icon: Icon, active }) => (
  <div
    className={`
      relative rounded-2xl p-4 border-r-4 transition
      ${active
        ? "bg-blue-100 border-blue-500"
        : "bg-gray-100 border-gray-300"
      }
    `}
  >
    <div className="absolute top-3 right-3 text-blue-900">
      <Icon size={20} />
    </div>

    <p className="text-[10px] uppercase font-bold text-gray-500 leading-tight">
      {title}
    </p>

    <p
      className={`mt-1 text-lg sm:text-xl font-black
        ${active ? "text-blue-900" : "text-gray-400"}`}
    >
      {value}
    </p>
  </div>
)

export default ComparisonCard
